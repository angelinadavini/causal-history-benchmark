...
| OLMo-2-1124-7B-Instruct | Third-family extension complete. Hidden-state movement was positive; the full-logit interval included zero. Setup and bridge-path checks passed. | SAME/DIFFERENT |
...
## Multi-event correction runs

Qwen2.5-3B-Instruct and OLMo-2-1124-7B-Instruct also completed a separate
256-episode run with an explicit correction event. Both retained a small
positive hidden-state endpoint after the correction. Neither produced a
positive full-logit endpoint. These runs are recorded under
